...
public class ComplexNavigationsCollectionsSplitQueryCalciteTest(ComplexNavigationsQueryCalciteFixture fixture)
    : ComplexNavigationsCollectionsSplitQueryRelationalTestBase<ComplexNavigationsQueryCalciteFixture>(fixture)
{
    ...
    public override async Task Include_inside_subquery(bool async)
        => Assert.Equal(
            SqliteStrings.ApplyNotSupported,
            (await Assert.ThrowsAsync<InvalidOperationException>(() => base.Include_inside_subquery(async))).Message);

    public override async Task Filtered_include_outer_parameter_used_inside_filter(bool async)
        => Assert.Equal(
            SqliteStrings.ApplyNotSupported,
            (await Assert.ThrowsAsync<InvalidOperationException>(() => base.Filtered_include_outer_parameter_used_inside_filter(async)))
            .Message);
    ...
    public override async Task Complex_query_issue_21665(bool async)
        => Assert.Equal(
            SqliteStrings.ApplyNotSupported,
            (await Assert.ThrowsAsync<InvalidOperationException>(() => base.Complex_query_issue_21665(async))).Message);
